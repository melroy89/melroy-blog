...
And each dot will separate the domain name, in this case we have:  
"blog", "melroy" and "org".

Example of a DNS request in an use-case diagram:
...
Assuming there is **no** DNS cache and you use a recursive DNS server (which you use, without you knowing).

The search will start at one of the root servers, as shown in the diagram above. The root server doesn't know the IP of `blog.melroy.org`, but responsds with a name server for the TLD (top-level domain), which is `.org` in our case.
...
Finally, the recursive resolver will contact that authoritative name server for "melroy.org" in order to find the IP address for "blog.melroy.org". The name server hopefully finds the IP address and returns it.

After that, the application (eg. web browser) can setup a request towards the IP address. In case of a web browser a request is made to the IP address and the web server will hopefully respond with the webpage content.
...
There can only be [13 root servers](https://www.iana.org/domains/root/servers) by design. Yes, you heard that right, only 13. Most of the internet root servers are in operated by USA as its origin.  
...
![DNS cache](/images/2024/12/dns_cache.webp "DNS local cache")
...
![Endless loop of DNS requests](/images/2024/12/dns_lookup.webp "Endless loop of DNS requests")
...
![DNS Lookup solved with glue records](/images/2024/12/dns_lookup_solved.webp "DNS requests resolved with glue records")
