...
This is a DNS blog serie of two parts. This is part 1; an introduction to DNS, including some examples and experiments on the command line.
...
And each dot will seperate the domain name, in this case: "blog", "melroy" and "org".

In order to know the name of the name server, a DNS forwarding request is sent to the root servers to find the name server for .org. Then the name server for .org is queried to find the name server for "melroy.org". Finally, the name server for "melroy.org" is queried to find the IP address for "blog.melroy.org".
...
> This trailing dot is criticial in zone files. Which is used in software like Bind or NSD.
>
...
> This trick only works if you have at least one dot in the domain name (wich is always the case today).
>
...
Then, the follow up question might be, which server does have the answers? These are the **authoritative name servers**. The root servers (mentioned earier) are all authoritative name servers. Software like Bind or NSD are used for authoritative name servers.

In part 2 of this DNS serie, we will actually use NSD to setup our own authoritative name server.
...
In part 2 of this DNS serie, we will setup our own name server and will create our own zone files.
...
We start with the most basic command:

```sh
dig facebook.com.
```

That should contain an `;; ANSWER SECTION:` section. With the IPv6 address of Facebook.
