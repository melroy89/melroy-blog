...
> Redis is an in-memory data structure store, used as a database and/or as a cache.
> It's used for storing and retrieving data in a fast and efficient manner from memory rather than from disk.
>
> Redis is used a lot in applications to speed up the application (eg. caching).
